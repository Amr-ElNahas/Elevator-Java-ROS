...
public class Elevator {
	...
	/*
	 * public void init(){ setTitle("graphing buttons "); setBounds(0,0 , 100,
	 * 100); panel = new JPanel(); panel.setPreferredSize(new
	 * Dimension(this.getWidth(), getHeight())); add(panel); }
	 */
	...
	public static void openDoor() {
		System.out.println("doors are opening at floor " + currentFloor);
		door = Doors.OPEN;
	}

	public static void closeDoor() {
		System.out.println("doors are closing at floor " + currentFloor);
		door = Doors.CLOSED;
			...
			while (true) {
				try {
					synchronized (this) {
						...
						seconds.add(second);
						if (memory.isEmpty()) {
							// System.out.println("at t=" + second + " seconds,
							// memory usage = 0%");
							usage.add(0);
						} else {
							// System.out.println("at t=" + second + " seconds,
					...
					synchronized (this) {
						...
						for (Process proc : processes) {
							if (proc.pcb.state == Enums.State.RUNNING) {
								running = true;
								// System.out.println("at t=" + second + "
								// seconds, processor usage = 100%");
								usage.add(100);
								break;
							}
						}
